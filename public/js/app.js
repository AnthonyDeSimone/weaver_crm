 ...
 *  The order app.
 */
var app = angular.module('order', ['ngIdle']);
var form = null;
var api;
...
app.controller('FormController', function ($scope, $filter, $timeout, $http, Idle) {
    ...
    api = new ApiCalls(this.BASE_URL, $http);
    ...
    api.addCall("keepalive", "post", "sales_orders/{0}/in_use", 5, false);
    api.log_activity = false;


    this.idCase = function(str) {
        return str.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase().replace(/[^a-z]+/g, '-').replace(/(^-+)|(-+$)/g, '');
    };
    ...
    this.getFeatureString = function () {
        if (form.options.feature == 'Custom') {
            ...
            return form.features[form.options.feature];
        }
    };
     ...
     */
    this.halfIncrementRange = function (component, key) {
        var max = component.max || Infinity;
        var min = component.min || 0;
        var value = component[key] || min;
        if (value < max) {
            component[key] = parseInt(parseFloat(value) + 1);
        }
    };
     ...
     */
    this.halfDecrementRange = function (component, key) {
        var min = component.min || 0;
        var value = component[key] || min;
        if (value > min) {
            value = parseInt(parseFloat(value) - 1);
            if(value == 2 && min == 2.5){
              value = 2.5
            }
    ...
    this.incrementRange = function (component, key) {
        ...
        var value = component[key] || min;
        if (value < max) {
            component[key] = parseInt(value) + 1;
        }
    };
    ...
    this.getAdditions = function () {
        ...
        form.fees.sales_tax = form.fees.sales_tax || 7.0
    ...
    this.startWatching = function () {
        ...
        form.clearShutterColorWatch = $scope.$watch(
            function(scope) {
                return form.getSelectedShutterOptions();
            },
            form.checkShutterColor
        )
    };

    form.checkShutterColor = function(num_selected) {
        let component = form.getComponentByPath('Paint & Stain', 'Paint', 'Shutter Color');
        if (component) {
            $timeout(function() {
                component.show = num_selected > 0;
            }, 0);
        }

        let component2 = form.getComponentByPath('Paint & Stain', 'Stain', 'Shutter Color');
        if (component2) {
            $timeout(function() {
                component2.show = num_selected > 0;
            }, 0);
        }
    };

    form.getSelectedShutterOptions = function() {
        var accessories = form.getSubsectionByPath('Windows', 'Window Accessories');
        var items = [];
        if (accessories && accessories.components && accessories.components.length && accessories.show) {
            for (var i = 0; i < accessories.components.length; i++) {
                var component = accessories.components[i];
                if (component.name.match(/shutter/i) && form.getQuantity(component)) {
    ...
    this.invoice = function () {
        window.onbeforeunload = null;
        window.location = form.BASE_URL + "sales_orders/" + form.order_id + ".pdf?timestamp=" + new Date().getTime();
    };
    ...
    form.loadJSON = function (data, id) {
        ...
        var pass1 = function () {
            ...
            //Added by Anthony 5/13
            form.sales_data = data.sales_data

            // REMOVED - Manually set the estimated time to avoid 60 minutes.
        ...
        var pass2 = function () {
            $timeout(pass1, 0).then(function () {
                $timeout(pass3, 500).then(function () {
                    $timeout(function () {
                        form.options = opts;
                    }, 800).then(function () {
                        $timeout(pass4, 800).then(function () {
                            $timeout(pass5, 1000);
                        });
                    });
                });
    ...
    this.getDisplayImage = function (component) {
        ...
        if (!component.value && (!component.options || component.options.length == 0)) {
            return null;
        }
...
////////////////////////////////////////////////////////////////////////////////
    ...
    this.getPaintStain = function () {
        if (form.options.feature.match(/Vinyl/)) return 'Vinyl';
        return form.paint_stain;
    };
    ...
    this.halfNumericUp = function (component, sub) {
        form.halfIncrementRange(component, 'value');
    }

    this.halfNumericDown = function (component, sub) {
        form.halfDecrementRange(component, 'value');
    }    
    ...
    this.findWarnings = function () {
        ...
        //         form.addWarning("Feature", "Solid Pine feature is chosen, but the orientation has not been picked.");
        //     }
        //     if (!form.options.side_out) {
        //         form.addWarning("Feature", "Solid Pine feature is chosen, but the side out has not been picked.");
        //     }
        // }
    ...
    $timeout(function () {
        ...
        form.is_admin_user = window.is_admin_user;
    }, 0);
    ...
    this.isaPavillion = function(){
        return (form.options.style == 'Dublin' || form.options.style == 'Timber Ridge' || form.options.style == 'Bradford')
    }

    
    this.isAStudio = function(){
        console.log(form.options.style.style)
        return (form.options.style == 'Studio')
    }    

    this.showVisualEditor = function (){
        return !(form.options.style == 'Dublin' || form.options.style == 'Timber Ridge' || form.options.style == 'Bradford')
    }
    ...
    IdleProvider.idle(25 * 60); // in seconds
    ...
    $rootScope.$on('IdleStart', function () {
        if (form.order_id) {
            jQuery('#idleCountdown').html('');
            jQuery('#idleModal').trigger('openModal');
        }
    });

    $rootScope.$on('IdleWarn', function (e, countdown) {
        if (form.order_id) {
            jQuery('#idleCountdown').html('<strong>Warning</strong>: You will be kicked out of this form in ' + countdown + ' seconds');
            if ((countdown % 30) === 0) {
                api.makeCall("keepalive", null, [form.order_id]);
            }
        }
    });
    ...
    $rootScope.$on('IdleEnd', function () {
        if (form.order_id) {
            api.makeCall("keepalive", null, [form.order_id]);
            jQuery('#idleModal').trigger('closeModal');
        }
    });
